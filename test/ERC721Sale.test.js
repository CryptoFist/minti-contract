const MintiERC721Token = artifacts.require("MintiERC721Token");
const MintiToken = artifacts.require("MintiToken");
...
describe("Sale", function() {
    ...
    var mintierc721token_contract;
    var transferproxy_contract;
    var servicefee_contract;
    var servicefeeproxy_contract;
    var salenonceholder_contract;
    var sale_contract;
    var mintitoken_contract;

    before(async () => {
        accounts = await web3.eth.getAccounts();
        
        await MintiERC721Token.new(
            "Minti721",
            "Minti721",
            accounts[0], // admin
            accounts[1], // signer
            "https://api-testnet.minti.co/contractMetadata/{address}",
            "https://ipfs.minti.co",
            { from: accounts[0] }
        ).then((instance) => {
            mintierc721token_contract = instance;
        });
        ...
        await MintiToken.new({ from: accounts[1] }).then(function (instance) {
            mintitoken_contract = instance;
        });
        ...
        await servicefee_contract.setMintiTokenContract(mintitoken_contract.address, { from: accounts[1] })
    ...
    const mint = async (minter, tokenId, fees = []) => {
        const contractAddressTokenIdSha = soliditySha3(
            mintierc721token_contract.address,
            tokenId,
            minter,
        );
        ...
        await mintierc721token_contract.mint(
            tokenId,
    ...
    describe("buy", () => {
        it("works and costs something (minter:6)", async () => {
            ...
            await mintierc721token_contract.setApprovalForAll(
                transferproxy_contract.address,
                true,
                { from: accounts[6] }
            );
            ...
            const saleContractBuySignature = soliditySha3(
                mintierc721token_contract.address, // token
                tokenId, // tokenId
            ...
            await sale_contract.setApproveSaleDate(mintierc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[6] })
            ...
            await sale_contract.buy(
                mintierc721token_contract.address, //IERC721 token,
                tokenId, //uint256 tokenId HAS TO BE THE PASSED TOKEN ID OF A MINTED TOKEN (MintiERC721Token)
            ...
            // Owner changed
            const newOwner = await mintierc721token_contract.ownerOf(tokenId);
        ...
        it("fails without minted token owner approval (minter:3)", async () => {
            ...
            // The owner of the token has to approve the transferproxy for sale == settings something on sale
            // DISABLED TO TEST --> await mintierc721token_contract.setApprovalForAll(transferproxy_contract.address, true, {from: accounts[3]});
            ...
            await sale_contract.setApproveSaleDate(mintierc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[3] })
        ...
        it("ApproveAll only needed once per minter (minter:4)", async () => {
            ...
            await sale_contract.setApproveSaleDate(mintierc721token_contract.address, tokenId1, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[4] })

            // Sold one token
            await sale_contract.buy(
                mintierc721token_contract.address, //IERC721 token,
                tokenId1, //uint256 tokenId HAS TO BE THE PASSED TOKEN ID OF A MINTED TOKEN (MintiERC721Token)
            ...
            const newOwner = await mintierc721token_contract.ownerOf(tokenId1);
            assert.equal(accounts[7], newOwner);

            await sale_contract.setApproveSaleDate(mintierc721token_contract.address, tokenId2, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[4] })

            // Sold two tokens
            await sale_contract.buy(
                mintierc721token_contract.address, //IERC721 token,
                tokenId2, //uint256 tokenId HAS TO BE THE PASSED TOKEN ID OF A MINTED TOKEN (MintiERC721Token)
                accounts[4], //address payable owner, the owner of the token from who we are buying the token from
                saleApprovalSignature2.signature, //signtaure
                { from: accounts[8], value: messageValue }
            );

            const newOwner2 = await mintierc721token_contract.ownerOf(tokenId2);
        ...
        it("does not work if prev owner try to approve", async () => {
            ...
            // Owner changed
            let newOwner = await mintierc721token_contract.ownerOf(tokenId);
            assert.equal(accounts[7], newOwner);
        ...
        it("can't buy token again once it was bought.", async () => {
            ...
            let thrownError;
            try {
                await sale_contract.setApproveSaleDate(mintierc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[8] })
            } catch (error) {
                thrownError = error;
            }
        ...
        it("can't buy if the token is not open to sale", async () => {
            ...
            await sale_contract.setApproveSaleDate(mintierc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-05-30')), { from: accounts[6] })
        ...
        it("can't set sale date if not owner", async () => {
            ...
            try {
                await sale_contract.setApproveSaleDate(mintierc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-05-30')), { from: accounts[5] })
            } catch (error) {
                thrownError = error;
            }
