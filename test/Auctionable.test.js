...
describe("ERC721Auction", function() {
    ...
    before(async () => {
        ...
        await MintiToken.new({ from: accounts[0] }).then(function (instance) {
        ...
        await servicefee_contract.setMintiTokenContract(MintiToken_contract.address, { from: accounts[0] })

        await AuctionMock.new(
...
/*        it("creating auction not working if caller is not the owner of the token", async () => {
        ...
        it("result auction", async () => {
            ...
            assert.equal(await MintiERC721Token_contract.ownerOf(tokenId, {from: accounts[0]}), accounts[9]);
