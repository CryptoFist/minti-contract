...
const { getProvider } = require('./helpers/utils');
const AuctionableArtifact = require('../artifacts/contracts/Auctionable.sol/Auctionable.json');
const ERC721MockArtifact = require('../artifacts/contracts/mocks/ERC721Mock.sol/ERC721Mock.json');
const ERC20MockArtifact = require('../artifacts/contracts/mocks/ERC20Mock.sol/Minti.json');
const { expect } = require('chai');

const [alice, bob, carl] = getProvider().getWallets();

describe('Auctionable', async () => {
	let minti, nft;
	let auction;
	...
	beforeEach(async () => {
		...
		await minti.transfer(bob.address, BigNumber.from(1000).mul(decimalsPowed));
	});
	...
	describe('bid', async () => {
		...
		it('Transfer the tokens to contract checking', async () => {
			...
			await minti.approve(auction.address, BigNumber.from(100).mul(decimalsPowed)); // 100 minti

			await auction.bid(
				BigNumber.from(1),
				BigNumber.from(100).mul(decimalsPowed)
			);
			await minti.balanceOf(alice.address).then(balance => {
