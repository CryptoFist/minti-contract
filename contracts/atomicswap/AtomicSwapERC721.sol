...
contract AtomicSwapERC721 is IERC721Receiver {
    ...
    modifier onlyEmptySwaps(bytes32 _swapID) {
        require(swapStates[_swapID] == States.EMPTY, "Swap is not empty");
        _;
    }
    ...
    modifier onlyExpirableSwaps(bytes32 _swapID) {
        require(swaps[_swapID].timelock <= block.timestamp, "Swap is not expired");
        _;
    }
    ...
    modifier onlyWithdrawer(bytes32 _swapID) {
        require(msg.sender == swaps[_swapID].withdrawer, "Caller is not the withdrawer");
        _;
    }

    modifier onlyProposer(bytes32 _swapID) {
        require(msg.sender == swaps[_swapID].proposer, "Caller is not the proposer");
        _;
    }
    ...
    function createProposal(
        ...
        require(
            ...
            "Caller is not token owner and/or contract not approved"
        );
