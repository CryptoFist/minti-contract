...
import "../interfaces/IMintiToken.sol";

/**
 * @notice Service Fee contract for Minti NFT Marketplace
 */
contract ServiceFee is AccessControl {
    using Address for address;

    /// @notice service fee contract
    IMintiToken public mintiTokenContract;
    ...
    event MintiTokenContractUpdated(address mintiTokenContract);
    ...
    */
     ...
     */
    function setMintiTokenContract(address _mintiTokenContract) onlyAdmin external {
        require(
            _mintiTokenContract != address(0),
            "ServiceFee.setMintiTokenContract: Zero address"
        );
        mintiTokenContract = IMintiToken(_mintiTokenContract);
        emit MintiTokenContractUpdated(_mintiTokenContract);
    }
    ...
    function getSellServiceFeeBps(address _seller, bool _isSecondarySale) external view onlyProxy returns (uint256) {
        ...
        uint256 balance = mintiTokenContract.balanceOf(_seller);
    ...
    function getBuyServiceFeeBps(address _buyer) onlyProxy external view returns (uint256) {
        ...
        uint256 balance = mintiTokenContract.balanceOf(_buyer);
